...
def _route(unrouted):
    ...
    try:
        app.logger.debug("stl: before >>packages.PackageManager.extract(...)<<")
        metadata, pmd = packages.PackageManager.extract(unrouted.id, unrouted.packaging_format)
        app.logger.debug("stl: after >>packages.PackageManager.extract(...)<<")
    except packages.PackageException as e:
        app.logger.debug("stl: in >>except packages.PackageException<<")
        ...
        if app.config.get("KEEP_FAILED_NOTIFICATIONS", False):
            ...
            failed.save()
            app.logger.debug(u"stl: Routing - Notification:{y} stored as a Failed Notification".format(y=unrouted.id))
    ...
    app.logger.debug("stl: before >>unrouted.match_data()<<")
    ...
    if pmd is not None:
        app.logger.debug("stl: before >>match_data.merge()<<")
